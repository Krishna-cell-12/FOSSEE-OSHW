...
let LedBarGraphElement = class LedBarGraphElement extends lit_1.LitElement {
    ...
    render() {
      ...
      >
        ...
        ${segments.map((index) => (0, lit_1.svg) `<rect x="2.5" y="${0.4 + index * 2.54}" width="5" height="1.74" fill="${values[index] ? (palette?.[index] ?? color) : offColor}"/>`)}
      </svg>
    `;
    }
};
...
exports.LedBarGraphElement = LedBarGraphElement = __decorate([
    (0, decorators_js_1.customElement)('wokwi-led-bar-graph')
], LedBarGraphElement);
